...
var convertUbuntuCmd = &cobra.Command{
	Use:   "ubuntu",
	Short: "Convert Vulnerability dictionary from Ubuntu",
	Long:  `Convert Vulnerability dictionary from Ubuntu`,
...
var supportUbuntuVers = []string{"14", "16", "18", "19", "20", "21"}
...
func convertUbuntu(_ *cobra.Command, _ []string) (err error) {
	...
	results, err := fetcher.FetchUbuntuFiles(supportUbuntuVers)
	if err != nil {
	...
	for _, r := range results {
		...
		for _, def := range models.ConvertUbuntuToModel(&ovalroot) {
	...
	if err := setLastUpdatedDate("goval-dictionary/ubuntu"); err != nil {
